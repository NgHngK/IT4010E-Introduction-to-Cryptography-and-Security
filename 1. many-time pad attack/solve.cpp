#include <iostream>
#include <string>
#include <vector>
using namespace std;

vector<unsigned char> hexToBytes(string hex) {
    vector<unsigned char> bytes;
    for (int i = 0; i < hex.length(); i += 2) {
        string byteString = hex.substr(i, 2);
        unsigned char byte = (unsigned char)stoi(byteString, nullptr, 16);
        bytes.push_back(byte);
    }
    return bytes;
}

bool checkByte(unsigned char b) {
    if (b >= 65 && b <= 90) return true;
    if (b >= 97 && b <= 122) return true;
    if (b == 32) return true;
    return false;
}

vector<int> recoverKey(vector<vector<unsigned char>> ciphertexts) {
    int maxLen = 0;
    for (int i = 0; i < ciphertexts.size(); i++) {
        if (ciphertexts[i].size() > maxLen) {
            maxLen = ciphertexts[i].size();
        }
    }

    vector<int> key(maxLen, -1);
    int space = 32;

    for (int i = 0; i < maxLen; i++) {
        int bestCount = -1;

        for (int j = 0; j < ciphertexts.size(); j++) {
            if (i >= ciphertexts[j].size()) {
                continue;
            }

            int count = 0;
            unsigned char c1 = ciphertexts[j][i];
            int keyFake = c1 ^ space;

            for (int k = 0; k < ciphertexts.size(); k++) {
                if (k == j || i >= ciphertexts[k].size()) {
                    continue;
                }
                unsigned char c2 = ciphertexts[k][i];
                unsigned char plaintest = c2 ^ keyFake;
                if (checkByte(plaintest)) {
                    count++;
                }
            }

            if (count >= bestCount) {
                key[i] = keyFake;
                bestCount = count;
            }
        }
    }

    return key;
}

string decryptWithKey(vector<unsigned char> ct, vector<int> key) {
    string result = "";
    for (int i = 0; i < ct.size(); i++) {
        if (key[i] != -1) {
            char c = ct[i] ^ key[i];
            result += c;
        } else {
            result += '?';
        }
    }
    return result;
}

int main() {
    string ciphertext1 = "";
    string ciphertext2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f";
    string ciphertext3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb";
    string ciphertext4 = "";
    string ciphertext5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070";
    string ciphertext6 = "";
    string ciphertext7 = "";
    string ciphertext8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3";
    string ciphertext9 = "";
    string ciphertext10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83";
    string targetCiphertext = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904";

    vector<vector<unsigned char>> ciphertexts;
    ciphertexts.push_back(hexToBytes(ciphertext1));
    ciphertexts.push_back(hexToBytes(ciphertext2));
    ciphertexts.push_back(hexToBytes(ciphertext3));
    ciphertexts.push_back(hexToBytes(ciphertext4));
    ciphertexts.push_back(hexToBytes(ciphertext5));
    ciphertexts.push_back(hexToBytes(ciphertext6));
    ciphertexts.push_back(hexToBytes(ciphertext7));
    ciphertexts.push_back(hexToBytes(ciphertext8));
    ciphertexts.push_back(hexToBytes(ciphertext9));
    ciphertexts.push_back(hexToBytes(ciphertext10));

    vector<unsigned char> target = hexToBytes(targetCiphertext);

    vector<int> key = recoverKey(ciphertexts);

    cout << "Decrypted messages:" << endl;
    cout << "===================" << endl;
    for (int i = 0; i < ciphertexts.size(); i++) {
        string plaintext = decryptWithKey(ciphertexts[i], key);
        cout << i + 1 << ": " << plaintext << endl;
    }

    string targetPlaintext = decryptWithKey(target, key);
    cout << endl << "Target message:" << endl;
    cout << "===================" << endl;
    cout << targetPlaintext << endl;

    return 0;
}
