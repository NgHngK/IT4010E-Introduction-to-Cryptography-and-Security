...
ciphertexts = [ciphertext1, ciphertext2, ciphertext3, ciphertext4, ciphertext5, ciphertext6, ciphertext7, ciphertext8, ciphertext9, ciphertext10]
# Convert hex to bytes
for i in range(len(ciphertexts)):
    ciphertexts[i] = bytes.fromhex(ciphertexts[i]) 

target = bytes.fromhex(target_ciphertext)
...
def recover_space_trick(ciphertexts):
    ...
    for i in range(L): # Consider each ciphertext in an array
        best_count = -1
        for j in range(len(ciphertexts)): # Consider each byte of the ciphertext
            if  i >= len(ciphertexts[j]): # If the byte index exceeds the length of the ciphertext, skip
                continue
            ...
            for k in range((len(ciphertexts))): # xor key with other ciphertexts
...
plaintexts = []
key = recover_space_trick(ciphertexts)
for i in range(len(ciphertexts)):
    plaintexts.append(decrypt_with_key(ciphertexts[i], key))
plaintexts.append(decrypt_with_key(target, key))
print(plaintexts)
for i in plaintexts:
    print(repr(i))

print("---------------------------")
print(repr(plaintexts[-1]))
